...
JAR_FILE="${APP_DIR}/membership-internal/target/membership-internal-1.0.0.jar"
...
echo -e "\n${BLUE}===============================================${NC}"
echo -e "${BLUE}      会员卡管理系统 - 内部管理平台部署脚本${NC}"
echo -e "${BLUE}===============================================${NC}"
...
cd "${APP_DIR}/membership-internal"
...
if curl -s http://localhost:8080/auth/login > /dev/null 2>&1; then
    ...
    echo -e "    ${RED}✗ 服务启动失败！查看日志: ${LOG_DIR}/internal-backend.log${NC}"
    exit 1
fi
...
# ------------------------------------------------------------------------------
echo -e "\n${GREEN}===============================================${NC}"
echo -e "${GREEN}           内部管理平台部署完成！${NC}"
echo -e "${GREEN}===============================================${NC}"
echo -e "\n${BLUE}服务信息:${NC}"
echo -e "  ├─ 服务名称: ${APP_NAME}"
echo -e "  ├─ 服务端口: 8080"
echo -e "  ├─ 服务地址: http://localhost:8080"
echo -e "  ├─ PID文件: ${PID_FILE}"
echo -e "  └─ 日志文件: ${LOG_DIR}/internal-backend.log"
echo -e "\n${BLUE}管理命令:${NC}"
echo -e "  ├─ 启动:   nohup java ${JAVA_OPTS} -jar ${JAR_FILE} > ${LOG_DIR}/internal-backend.log 2>&1 &"
echo -e "  ├─ 停止:   kill -TERM \$(cat ${PID_FILE})"
echo -e "  └─ 日志:   tail -f ${LOG_DIR}/internal-backend.log"
