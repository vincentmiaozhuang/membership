...
if (typeof document !== 'undefined') {
  ...
  style.textContent = `
    ...
    .summary-row.productCustomer {
      background-color: #e6f7ff !important;
    }
    .summary-row.product {
      background-color: #a6e3ffff !important;
    }
    .summary-row.total {
...
const StatsManagement = () => {
  ...
  const fetchStatsList = async () => {
    ...
    try {
      ...
      console.log('发起请求到:', '/stats/list')
      const response = await request.get('/stats/list', { params })
      console.log('获取统计数据成功:', response)
      console.log('响应类型:', typeof response)
      console.log('响应长度:', response ? response.length : 0)
      
      // 计算合计数据
      const processedData = calculateTotals(response)
      
      // 后端返回的是直接的列表数据
      setStatsList(processedData)
      setTotal(processedData.length)
      console.log('设置统计数据:', processedData)
    } catch (error) {
  ...
  const handleExport = () => {
    ...
    // 准备导出数据
    const exportData = statsList.map(item => ({
      '产品': item.productName || '',
      '客户': item.customerName || '',
      '供应商': item.supplierName || '',
      '充值状态': item.status || '',
      '产品面值': item.productFacePrice || '',
      '成本单价': item.supplierPrice || '',
      '售卖单价': item.customerPrice || '',
      '数量': item.rechargeCount || 0,
      '成本金额': item.costAmount || 0,
      '销售金额': item.customerAmount || 0
    }))
    ...
    const fileName = `统计数据_${timestamp}.xlsx`
  ...
  const calculateTotals = (data) => {
    ...
    // 过滤出状态为成功的数据
    const successData = data.filter(item => item.status === '成功')

    // 按产品ID和客户ID分组
    const productCustomerGroups = {}
    successData.forEach(item => {
      const key = `${item.productId}_${item.customerId}`
      if (!productCustomerGroups[key]) {
        productCustomerGroups[key] = {
      ...
      productCustomerGroups[key].rechargeCount += item.rechargeCount
      productCustomerGroups[key].costAmount = Number((productCustomerGroups[key].costAmount + (item.costAmount || 0)).toFixed(2))
      productCustomerGroups[key].customerAmount = Number((productCustomerGroups[key].customerAmount + (item.customerAmount || 0)).toFixed(2))
    })
    ...
    successData.forEach(item => {
      ...
      productGroups[key].rechargeCount += item.rechargeCount
      productGroups[key].costAmount = Number((productGroups[key].costAmount + (item.costAmount || 0)).toFixed(2))
      productGroups[key].customerAmount = Number((productGroups[key].customerAmount + (item.customerAmount || 0)).toFixed(2))
    })
    ...
    successData.forEach(item => {
      totalSummary.rechargeCount += item.rechargeCount
      totalSummary.costAmount = Number((totalSummary.costAmount + (item.costAmount || 0)).toFixed(2))
      totalSummary.customerAmount = Number((totalSummary.customerAmount + (item.customerAmount || 0)).toFixed(2))
    })

    // 处理原始数据，添加合计行
    const processedData = []
    const addedProductCustomerKeys = new Set()
    ...
    sortedData.forEach(item => {
      ...
      // 检查是否需要添加产品ID和客户ID合计
      const productCustomerKey = `${item.productId}_${item.customerId}`
      if (item.status === '成功' && !addedProductCustomerKeys.has(productCustomerKey)) {
        // 检查下一个项目是否属于同一产品和客户
        const nextItem = sortedData[sortedData.indexOf(item) + 1]
        if (!nextItem || nextItem.productId !== item.productId || nextItem.customerId !== item.customerId) {
          const summary = productCustomerGroups[productCustomerKey]
          if (summary) {
            processedData.push({
              ...summary,
              id: `pc_${productCustomerKey}`,
              supplierName: '供应商合计',
              status: '成功',
              productFacePrice: '',
              supplierPrice: '',
              customerPrice: '',
              isSummary: true,
              summaryType: 'productCustomer'
            })
            addedProductCustomerKeys.add(productCustomerKey)
          }
        }
      }
      ...
      if (item.status === '成功' && !addedProductKeys.has(productKey)) {
        // 检查下一个项目是否属于同一产品
        const nextItem = sortedData[sortedData.indexOf(item) + 1]
        if (!nextItem || nextItem.productId !== item.productId) {
          const summary = productGroups[productKey]
          if (summary) {
            processedData.push({
              ...summary,
              id: `p_${productKey}`,
    ...
    if (successData.length > 0) {
      processedData.push({
        ...
        supplierPrice: '',
        customerPrice: '',
        rechargeCount: totalSummary.rechargeCount,
        costAmount: totalSummary.costAmount,
        customerAmount: totalSummary.customerAmount,
        isSummary: true,
        summaryType: 'total'
      })
    }
    ...
    <div>
        ...
        <Row gutter={12} align="middle">
          ...
          <Col span={4} style={{ textAlign: 'right' }}>
            <Space>
              ...
              <Button onClick={handleReset} style={{ width: 80, height: 32 }}>
                重置
              </Button>
              <Button 
                type="primary" 
      ...
      <div style={{ overflowX: 'auto' }}>
        <Table
          ...
          rowClassName={(record) => {
            if (record.isSummary) {
              return `summary-row ${record.summaryType}`
            }
            return ''
          }}
          pagination={{
